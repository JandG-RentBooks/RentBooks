...
@Component({
  ...
  styleUrls: ['./cover-type.component.scss']
})
export class CoverTypeComponent implements OnInit {
    ...
    params: { url: string, pages: number[], perPage: number, searchValue: string } = {
        url: `${API_URL}admin/cover-type?page=1`,
        pages: [5, 10, 20, 50],
        perPage: 10,
        searchValue: '',
    }
    ...
    constructor(private coverTypeService: CoverTypeService, private sharedService: SharedService) {
        this.index()
    }
    ...
    perPageOnChange(e: any): void {
        let url = `${API_URL}admin/cover-type?page=1`
        this.paginate(url)
    }
    ...
    index(): void {
        this.coverTypeService.index(this.params).subscribe({
            next: data => {
                console.log(data)
    ...
    destroy(e: any): void {
        ...
        this.coverTypeService.destroy(this.selectedItem.id).subscribe({
            next: data => {
