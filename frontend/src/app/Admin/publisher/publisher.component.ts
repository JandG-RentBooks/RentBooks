...
export class PublisherComponent {
    ...
    params: { url: string, pages: number[], perPage: number, searchValue: string } = {
        url: `${API_URL}admin/publisher?page=1`,
        pages: [5, 10, 20, 50],
        perPage: 10,
        searchValue: '',
    }
    ...
    constructor(private publisherService: PublisherService, private sharedService: SharedService) {
        this.index()
    }
    ...
    perPageOnChange(e: any): void {
        let url = `${API_URL}admin/publisher?page=1`
        this.paginate(url)
    }
    ...
    index(): void {
        this.publisherService.index(this.params).subscribe({
            next: data => {
                console.log(data)
    ...
    destroy(e: any): void {
        ...
        this.publisherService.destroy(this.selectedItem.id).subscribe({
            next: data => {
