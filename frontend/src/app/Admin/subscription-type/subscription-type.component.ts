...
export class SubscriptionTypeComponent {
    ...
    params: { url: string, pages: number[], perPage: number, searchValue: string } = {
        url: `${API_URL}admin/subscription-type?page=1`,
        pages: [5, 10, 20, 50],
        perPage: 10,
        searchValue: '',
    }
    ...
    perPageOnChange(e: any): void {
        let url = `${API_URL}admin/subscription-type?page=1`
        this.paginate(url)
    }
    ...
    index(): void {
        this.subscriptionTypeService.index(this.params).subscribe({
            next: data => {
                console.log(data)
    ...
    destroy(e: any): void {
        ...
        this.subscriptionTypeService.destroy(this.selectedItem.id).subscribe({
            next: data => {
